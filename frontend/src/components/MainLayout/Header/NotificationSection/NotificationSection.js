...
import Transitions from '../../../uiComponents/extended/Transitions';
...
const NotificationSection = () => {
    ...
    const handleToggle = () => {
        ...
        socket.emit("check_all_notifications", user?.result?._id);
    };
    ...
    useEffect(() => {
        socket.emit("initial_data", user?.result?._id);
    ...
    const getData = (notifications) => {
        ...
        readNotif = notifications?.length - unreadNotif;

        console.log('readNotif: ' + readNotif);
        console.log('unreadNotif: ' + unreadNotif);
    };

    const changeData = () => socket.emit("initial_data", user?.result?._id);
